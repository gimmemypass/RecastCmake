...
int dtTileCache::getTilesAt(const int tx, const int ty, dtCompressedTileRef* tiles, const int maxTiles) const 
{
	...
	while (tile)
	{
		...
		int id = decodeTileIdTile(tileRef);
		std::string debug;
		debug.append("GetTilesAt");
		debug.append(" ");
		debug.append(address.str());
		debug.append(" ");
		debug.append(std::to_string(id));
		debug.append(" ");
		debug.append(std::to_string(tx));
		debug.append(" ");
		debug.append(std::to_string(tile->header->tx ));
		debug.append(" ");
		debug.append(std::to_string(ty));
		debug.append(" ");
		debug.append(std::to_string(tile->header->ty));
		debug.append(" ");
...
{
	//std::string debug = "m_nupdate ";
	//debug.append(std::to_string(m_nupdate));
	//debug.append(" m_updateType : ");
	//debug.append(std::to_string(m_updateType));
	//debug.append(" m_nreqs :");
	//debug.append(std::to_string(m_nreqs));
	//DebugCPP::Log(debug);
	//switch (m_updateType)
	//{
	//	case 0 :
	//		DebugCPP::Log("m_updateType == UPDATE_OBSTACLES");
	//		break;
	//	case 1 :
	//		DebugCPP::Log("m_updateType == UPDATE_FLAGS");
	//		break;
	//	default:
	//		break;
	//}
	...
	if (m_nupdate == 0)
	{
		...
		for (int i = 0; i < m_nreqs; ++i)
		{
			...
			else if (req->action == REQUEST_REMOVE)
			{
				...
				DebugCPP::Log("request_??");
			}
		}

		if (m_nupdate > 0)
		{
			DebugCPP::Log("set UPDATE_OBSTACLES updateType");
	...
	if (m_nupdate == 0)
	{
		...
		if (m_nupdate > 0)
		{
			DebugCPP::Log("[DETOUR] set UPDATE_FLAGS updateType");
